...
**Local preview.38:** track tools and goal-specific driver feedback now share the main application and normal ADT Companion. Recorded-route section goals, scoped pass reviews and the whole-run quick review are implemented; no separate developer installation is needed. Accumulated learning across independent matched tests and live driving acceptance remain future work. See [the integrated guide](docs/TRACK_TOOLS.md). This local update has not been published.

**September 26, 2026 — active priority: finish Telemetry Intelligence 2.0.** Local preview.35 implements exact setup-test tracking, verified before/after changes, analysis provenance and consistent backward-travel filtering. Local preview.36 adds shared legal setup mappings and closes A1/A2/A7 for supported controls, including truthful holds for unsupported or rounded-away changes. Local preview.37 adds diagnosis by phase, speed and direction, linked to saved goals and pedal evidence, and holds blanket recommendations when conditions conflict. Next come accumulated matched car/driver tests and driving acceptance. Track concrete completion criteria in [Telemetry Intelligence 2.0](docs/TELEMETRY_INTELLIGENCE.md#intelligence-20-completion-plan--september-26). Preview.33 is the current public beta; later local previews have not been published. Older milestone notes below describe the roadmap's development history.
...
Local companion preview: desktop **0.9.0-preview.15** and **ADT Companion 0.4.0-preview.1** add a **Pit setup** tab for explicit Save & Apply Tune of a numeric plan staged in desktop AC Setup, with unique previous/changed setup files, current-setup readback and same-session Restore previous. Pit setup editing, stationary state, current car/baseline, editable values and no recording are required. Generation, capture and review do not apply car/FFB values. Manual export/load remains available. Real-game acceptance is pending; next work is the [first end-to-end pit test and refusal/recovery checks](docs/PIT_SETUP.md), plus the [recording and capture checklist](docs/COMPANION.md).
