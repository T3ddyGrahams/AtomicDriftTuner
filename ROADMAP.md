...
**September 26, 2026 — active priority: finish Telemetry Intelligence 2.0.** Local preview.35 implements exact setup-test tracking, verified before/after changes, analysis provenance and consistent backward-travel filtering. Next come the remaining setup-value audit findings, deeper phase/speed diagnosis, then accumulated matched car/driver tests and driving acceptance. Track concrete completion criteria in [Telemetry Intelligence 2.0](docs/TELEMETRY_INTELLIGENCE.md#intelligence-20-completion-plan--september-26). Preview.33 is the current public beta; later local previews have not been published. Older milestone notes below describe the roadmap's development history.
