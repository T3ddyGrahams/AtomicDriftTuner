...
**Local preview.26 — clearer recording progress:** Useful time adds up across shorter corners and drifts; you do not need one uninterrupted ten-second section. The front-response check now shows progress toward ten seconds total of normal cornering at 30+ km/h, separately from drifting with readable axle-slip signals. After 60 seconds of useful drift with reliable telemetry, **READY FOR PARTIAL REVIEW** lets you stop/save and review the evidence already available. Missing goal measurements stay listed and insufficient in the analysis. Full **READY TO REVIEW** still appears when all selected goal checks pass. The optional chime sounds once per recording. Analysis thresholds and tuning formulas are unchanged; update companion **0.4.0-preview.4** with the driving session closed, then launch a new session.

**Local preview.25 — partial physics import:** Readable suspension and other physics sections remain available when a separate section contains malformed text. In **AC Setup**, load the baseline or press **Refresh car physics**, then expand **Car physics — values and sources**. Any excluded section is identified. Nonstandard differential values remain unknown with a clearer explanation; ADT does not guess their scale. Car files and tuning formulas are unchanged. Includes the recording and matching-copy fixes; companion **0.4.0-preview.3** is unchanged.
