...
**Local preview.38 — track tools and quick driver feedback in the main app:** Open **Tuning Assistant → Before / After → Quick driver review** to answer Better / Same / Worse / Couldn't judge and save the suggested next step. **Track & sections** adds recorded-route maps, line/angle/gear goals, matched pass comparisons and section-specific feedback. Existing main settings, tunes and history are retained. Update **ADT Companion to 0.5.0-preview.1** with the driving session closed to capture positions, then start a new session. The normal Remote port remains 5190. See `docs/TRACK_TOOLS.md`; no separate developer application is required. Live driving acceptance is pending.

**Included from preview.35 — Telemetry Intelligence 2.0, first milestone:** A focused car setup test now records its exact intended values and verifies the captured before/after change. Wrong or extra changes, file-only edits and missing fields cannot validate the recommendation. New reviews also require improvement in the intended measurement and retain analysis versions. Use **Review one car setup change**, save/stage, apply in the pits, record again and inspect **Test verification** in Before / After. For your own experiment, Recorder has an explicit **This is my own car setup test** option. See `docs/TELEMETRY_INTELLIGENCE.md` for the workflow and remaining 2.0 milestones. Known backward travel is consistently excluded from clean drift, phase, pedal and gain evidence; angle/recovery observations are retained.

**Included from preview.34 — adjustable rev limits in gearing:** Standard adjustable engine limiters can now be read from your saved baseline and the car's definition. Open **AC Setup → Gearing**, select the setup you intend to drive, and use **Read car and suggest RPM range** before calculating again. ADT uses the resulting setup rev limit for its gearing estimates. It preserves the limiter when saving a final-drive change; it does not read live ECU/script overrides. If your current final drive is already the closest available match, there is no new gearing setup to save. See `docs/GEARING.md`.
...
**Start here after upgrading:** Close ADT before installing, then reopen and verify **0.9.0-preview.38**. Reload your intended baseline before generating/staging. For the new track tools, exit the AC driving session, use **Remote → Install / Update Companion** for **0.5.0-preview.1**, then start a new session. If updating the packaged SimHub/AZOM bridge, close SimHub first and restart afterwards; its logic has not changed in this update. Record a fresh baseline for the expanded evidence and source snapshots if upgrading from an older public version.
