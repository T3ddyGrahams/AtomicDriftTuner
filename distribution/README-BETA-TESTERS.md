...
**Local preview.10:** Open **Tuning Assistant → Pedal Evidence** after saving a run to inspect throttle, brake and clutch events alongside the surrounding car response. Select an event for its full explanation below the table. Assessments connect the evidence to the run's saved Desired Behavior; Before / After now flags changed pedal use before attributing improvement to the tune. Pedal telemetry comes directly from AC and does not require SimHub/AZOM. Start with the quick test in `docs/TELEMETRY_INTELLIGENCE.md`.
