...
**Start here after upgrading:** Close ADT before installing, then reopen and verify **0.9.0-preview.30**. Reload your intended baseline before generating/staging. If upgrading from preview.17, exit the AC driving session, use **Remote → Install / Update Companion** for **0.4.0-preview.4**, then start a new session. If using SimHub/AZOM, update the packaged bridge to **0.9.0-preview.30** with SimHub closed and restart SimHub. Record a fresh baseline for the expanded evidence and source snapshots.
