...
**Local preview.8:** Recording now allows up to five seconds for a brief AC telemetry outage to recover instead of immediately ending the run. The desktop, in-game companion and touchscreen display the waiting state and actual interruption reason. Longer outages preserve a partial run; stop reasons are retained in saved session JSON. Run another baseline → recommendation → comparison test to verify this on your rig. The existing companion and SimHub dashboard need no reinstall; start the updated desktop ADT and pair again.

The original combined FFB/car-setup workflow, touchscreen controls and clearer instructions from preview.7 remain. Open **Remote → START REMOTE → INSTALL SIMHUB DASHBOARD**, then launch **ADT Control Center** in SimHub Dash Studio and tap in the pairing code. Prepare the desktop recorder once, then use **Start run → Stop run → Save run** on the touchscreen. Its `/dash` page also works on a phone/tablet using the PC's LAN address. Full setup and troubleshooting are in `docs/TOUCHSCREEN.md`; the tuning walkthrough is in `docs/GUIDED_WORKFLOW.md`. History, Desired Behavior, gearing and numerical diagnosis/tuning calculations are preserved.
