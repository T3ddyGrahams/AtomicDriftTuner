...
- From local preview.25, malformed text or duplicate definitions exclude the affected physics section instead of erasing independent readable sections. For example, un-commented notes under `[DAMAGE]` need not hide valid `[BASIC]`, `[FRONT]`, `[REAR]` and `[ARB]` values. A broken header also leaves uncertain adjacent fields unknown; global values are never assigned to the next section. The details panel names the excluded section. File/section/field reading limits remain enforced.
- Base-file differential `POWER`/`COAST` values on ADT's supported 0–1 scale are displayed as percentages. Values outside that scale remain unknown, with their raw number shown when finite. For example, a raw value of `85` is not automatically interpreted as 85% or clamped to a locking value. Confirm the active pit setting separately; saved setup values and base physics use different representations.
- Ordinary packed data and an unambiguous unpacked `data` folder are supported. Both may be present if their complete supported INI/LUT/RTO/Lua file sets and bytes match. Preview.31 permits one narrow exception: finite `[GRAPHICS] DRIVEREYES` and `ON_BOARD_PITCH_ANGLE` values may differ in otherwise identical, unambiguous `car.ini` files. These are driver-seat camera position/pitch fields, as documented by the [CSP Lua SDK](https://github.com/ac-custom-shaders-patch/acc-lua-sdk/blob/main/common/ac_game.lua). Other graphics fields, mass, inertia, steering, FFB and all other supported content remain strictly checked; missing/extra, malformed, duplicate or nonfinite values do not get this exception. ADT reads one complete packed snapshot, records raw fingerprints for both copies and explains the camera-only difference. Later camera edits still invalidate an old preview until reloaded/recalculated. It does not infer which source is active, merge sources or modify any car/camera file.
