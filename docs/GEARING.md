...
**AC Setup → Gearing** compares final-drive choices for tight-corner and long-sweeper goals. ADT reads supported packed or unpacked car data and writes a separate setup when there is a change to test. Local preview.34 also reads standard adjustable engine limiters from the selected baseline.
...
A candidate reaching the setup rev limit in **either** requested section is excluded. Ranking first prefers a full fit for all requested sections, then minimizes the average squared proportional mismatch at both speed endpoints, weighting each section equally. Ties/negligible differences retain the current ratio. If no eligible preset exists, no recommendation is produced.

For a standard adjustable `ENGINE_LIMITER`, ADT combines the saved baseline selection with its `setup.ini` definition to obtain a percentage of `engine.ini [ENGINE_DATA] LIMITER`. The supported `SHOW_CLICKS` modes are 0 (saved percentage), 1 (saved value × STEP), and 2 (MIN + saved value × STEP). Missing `SHOW_CLICKS` defaults to 0. Both the definition and saved selection must be present; bounds, step alignment and whole click counts are checked. For example, a 7,900 RPM base at 100% gives a 7,900 RPM setup rev limit. Fixed-limit cars continue to use the base limiter. Unknown `LIMITER` aliases, list/curve mappings and ambiguous or incomplete definitions remain blocked.

This limit describes the **selected saved setup**, not an active ECU/script readback. Load that exact setup in AC before testing. If an older RPM target exceeds it, click **Read car and suggest RPM range**, or lower the upper RPM under **Advanced**. Reading the limit does not change the limiter or the installed car. Gearing export still changes only the final-drive selection and preserves the saved limiter.

For automatic RPM guidance, ADT reads `engine.ini [HEADER] POWER_CURVE` and a bounded, ordered, numeric local LUT. Its provisional rule samples base torque at 100-RPM intervals, computes proportional base power as RPM × torque, and selects the contiguous region around the sampled peak with at least 80% of that peak. Sampling stays inside the curve's domain, above the defined engine minimum (or 500 RPM when absent), and at or below 95% of the setup rev limit. At least a 500-RPM-wide region is required. No extrapolation or fabricated torque values are used.
...
Limiter mapping references: Content Manager's setup entry loader identifies `ENGINE_LIMITER` as percent and decodes saved values according to [CarSetupStepsMode](https://github.com/gro-ove/actools/blob/master/AcManager.Tools/Objects/CarSetupStepsMode.cs). The installed official Kunos SDK `formula_k` example uses `ENGINE_LIMITER` with MIN=95, MAX=100, STEP=1, SHOW_CLICKS=1. ADT applies that standard percentage to the base RPM limit; custom ECU/controller behavior is outside this calculation.
...
Regression fixtures cover joint ranking, limiter exclusion in either goal, packed/unpacked round trips, all three standard limiter display modes, invalid/missing limiter definitions, stale limiter selections and data, limiter preservation on export, fixed/individual/preset gearbox detection, stale second-gear or curve data, curve coverage and malformed input, legacy profiles, unit preferences, goal snapshots, overlapping exposure and recorded-speed eligibility. WPF checks exercise real handlers, over-limit targets and RPM refresh, both goal cards, persisted targets, conversion without drift, inherited units, invalidation, theme colors and narrow/portrait/wide layouts. Installed-car validation reads data and leaves the game and user setup files unchanged. Driving is still required to assess the result.
