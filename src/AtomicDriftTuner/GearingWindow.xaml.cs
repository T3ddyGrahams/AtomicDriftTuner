...
public partial class GearingWindow : Window
{
    ...
    private void Calculate_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var fit = next.FitsTarget ? "The predicted endpoints fit inside your target RPM band." : "No supported ratio fits your entire target RPM band. This is the closest endpoint match below the limiter; consider a different gear or speed/RPM range.";
            ResultText.Text = $"{title}\n\nIn gear {target.Gear}, at {speedRange}:\nCurrent {current.FinalDrive.Ratio:0.###}:1: {current.LowRpm:N0} → {current.HighRpm:N0} RPM\nSuggested {next.FinalDrive.Ratio:0.###}:1: {next.LowRpm:N0} → {next.HighRpm:N0} RPM\nTarget: {target.MinimumRpm:N0}–{target.MaximumRpm:N0} RPM\n\n{fit}\nRPM and torque multiplication change: {change:+0.0;-0.0;0.0}% in every gear.\nLimiter speed in gear {target.Gear}: {FormatSpeed(next.LimiterSpeedKmh)} {Unit} (no slip).\n\nRanking prefers a full RPM-band fit, then the smallest proportional mismatch at both speed endpoints. It excludes ratios that reach the limiter within this range.";
            SourceText.Text = $"Verified from unpacked car files. {_plan.Data.GearSource}; gear ratio {_plan.Data.GearRatio:0.####}:1.\n{_plan.Data.TyreSource}; nominal radius {_plan.Data.TyreRadius:0.###} m. Engine limiter {_plan.Data.LimiterRpm:N0} RPM.\nSaved final-drive index {current.FinalDrive.Index} → {next.FinalDrive.Index}. List order is preserved.";
            OptionsText.Text = string.Join("\n\n", _plan.Options.Select(x => $"{x.FinalDrive.Ratio:0.###}:1 • {x.FinalDrive.Label}\n{x.LowRpm:N0}–{x.HighRpm:N0} RPM • {(x.HighRpm >= _plan.Data.LimiterRpm ? "excluded: reaches limiter" : x.FitsTarget ? "fits target band" : "partial fit")}{(x.FinalDrive.Index == next.FinalDrive.Index ? " • suggested" : "")}{(x.FinalDrive.Index == current.FinalDrive.Index ? " • current" : "")}"));
