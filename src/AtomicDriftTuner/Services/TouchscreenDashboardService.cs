...
/// <summary>Creates the native SimHub WebPageItem dashboard using ADT's local touchscreen page.</summary>
public static class TouchscreenDashboardService
{
    ...
    public static string Render(int port)
    {
        var dashboard = JsonNode.Parse(Template)!;
        dashboard["Screens"]![0]!["Items"]![0]!["StartAddress"] = LocalAddress(port);
    ...
    public static string Install(string? simHubPath, int port)
    {
        var json = Render(port);
    ...
    private const string Template = """
{
  ...
  "Screens": [{
    ...
    "Items": [{
      ...
      "Name": "ADT touchscreen", "StartAddress": "http://127.0.0.1:5190/dash",
  ...
  "Metadata": {
    ...
    "Description": "Paired ADT touchscreen controls, run status and tuning guidance.",
