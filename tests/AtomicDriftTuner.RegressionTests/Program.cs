...
{
    FeedbackChecks.Run(Run, root);
    Console.WriteLine($"Failures: {failures}. Isolated fixtures: {root}");
    return failures == 0 ? 0 : 1;
}
if (args.Contains("--track"))
{
    TrackSectionChecks.Run(Run, root);
    Console.WriteLine($"Failures: {failures}. Isolated fixtures: {root}");
    return failures == 0 ? 0 : 1;
}
if (args.Contains("--driving-context"))
{
    DrivingContextChecks.Run(Run, root);
    Console.WriteLine($"Failures: {failures}. Isolated fixtures: {root}");
    return failures == 0 ? 0 : 1;
}
if (args.Contains("--setup-legality"))
{
    SetupLegalityChecks.Run(Run, root);
    Console.WriteLine($"Failures: {failures}. Isolated fixtures: {root}");
    return failures == 0 ? 0 : 1;
}
if (args.Contains("--intelligence-v2"))
{
    IntelligenceV2Checks.Run(Run, root);
    Console.WriteLine($"Failures: {failures}. Isolated fixtures: {root}");
    return failures == 0 ? 0 : 1;
}
...
Run("AZOM source guard rejects stale values and accepts target no-op", () =>
{
    ...
    method.Invoke(null, new object[] { plan, new AzomLiveSnapshot { Torque = 70 } });
        ...
        PartialPhysicsChecks.Run(Run, root);
        PackedArchiveChecks.Run(Run, root);
        MatchingCarDataChecks.Run(Run, root);
