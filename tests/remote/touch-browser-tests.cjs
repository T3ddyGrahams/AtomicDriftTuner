...
let commandDelay=0,evidenceReady=true,evidenceLimited=false,runId='fixture-run';
let dropCommandReply=false,carName='Example drift car';
const recorder=()=>({evidence:{state:evidenceReady?'ready':'more-evidence',readyToReview:evidenceReady,hasGoalLimitations:evidenceLimited,heading:evidenceReady?(evidenceLimited?'READY FOR PARTIAL REVIEW':'READY TO REVIEW'):'KEEP COLLECTING',neededEvidence:evidenceReady?(evidenceLimited?['Front response: 3.0 / 10 s total. Drive normal corners at 30+ km/h without drifting.','Axle-slip evidence: 2.0 / 10 s total.']:[]):['Need clean entries','Need clean transitions'],message:evidenceReady?(evidenceLimited?'Useful drift is ready for partial review. Stop and save, or continue for the missing goals. Front response: 3.0 / 10 s total. Drive normal corners at 30+ km/h without drifting.':'Enough evidence to review. Stop and save when ready.'):'Need clean entries',details:'Useful drift: 25 seconds; final analysis remains authoritative.'},setupMessage:'Current setup captured from CSP: 8 numeric values.',windowId:'a'.repeat(32),sessionId:runId,controlVersion:version.toString(16).padStart(64,'0'),car:'Example drift car',driver:'Test driver',state,canStart,canStop,canSave,samples:state==='ready'?0:3000,elapsedSeconds:state==='ready'?0:75,message:state==='ready'?'Ready to record.':state==='recording'?'Recording in ADT.':state==='unsaved'?'Stop complete. Save your run.':'Session saved.'});
...
(async()=>{
  ...
  evidenceLimited=true;runId='next-recording';
  await page.waitForFunction(()=>testReadyChimes===1);
  check(await page.locator('#evidenceHeading').innerText()==='READY FOR PARTIAL REVIEW'&&(await page.locator('#controlEvidence').innerText()).includes('normal corners')&&(await page.locator('#evidenceNeeded').innerText()).includes('Axle-slip'),'Partial review hides missing measurements');
  ...
  await page.waitForFunction(()=>document.getElementById('evidenceHeading').textContent==='READY TO REVIEW');
  check(await page.evaluate(()=>testReadyChimes)===1,'Full evidence replayed partial-review chime');
  await page.reload();
  await page.waitForFunction(()=>!document.getElementById('recordStop').disabled);
